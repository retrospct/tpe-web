...
const EventsList = ({ slice }: EventsListProps): JSX.Element => {
  const anchorLink = isFilled.keyText(slice.primary.anchor_link)
  ...
  return (
    <section data-slice-type={slice.slice_type} data-slice-variation={slice.variation} className="my-10 w-full">
      {slice.variation === 'default' && (
        <div id={anchorLink} className="flex w-full flex-col items-center justify-center">
          {isFilled.richText(slice.primary.title) && (
            <HeadingDivider richText={slice.primary.title} anchor={anchorLink} className="max-w-4xl" />
          )}
