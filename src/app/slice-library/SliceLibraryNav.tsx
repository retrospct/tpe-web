...
export default function SliceLibraryNav({ libraries }: { libraries: SliceLibrary[] }) {
  return (
    <div>
        ...
        <>
          ...
          <Disclosure.Panel className="space-y-1">
            {library.slices.map(({ model }) => (
              <NavElementSlice key={model.name} libraryName={library.name} slice={model} />
            ))}
          </Disclosure.Panel>
        </>
      )}
...
function NavElementSlice({ libraryName, slice }: { libraryName: string; slice: SharedSlice }) {
  return (
    <Disclosure as="div" className="space-y-1">
      {({ open }) => (
        <>
